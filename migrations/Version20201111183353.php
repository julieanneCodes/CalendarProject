 ...
 */
final class Version20201111183353 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, config_name_id INTEGER DEFAULT NULL, email VARCHAR(180) NOT NULL COLLATE BINARY, name VARCHAR(30) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_8D93D64940B5E26 FOREIGN KEY (config_name_id) REFERENCES user_config (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO user (id, config_name_id, email, name, password) SELECT id, config_id_id, email, name, password FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
        $this->addSql('CREATE INDEX IDX_8D93D64940B5E26 ON user (config_name_id)');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_527EDB25A76ED395 ON task (user_id)');
        $this->addSql('DROP INDEX UNIQ_8D93D649E7927C74');
        $this->addSql('DROP INDEX IDX_8D93D64940B5E26');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, config_name_id, email, name, password FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, email VARCHAR(180) NOT NULL, name VARCHAR(30) NOT NULL, password VARCHAR(255) NOT NULL, config_id_id INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, config_id_id, email, name, password) SELECT id, config_name_id, email, name, password FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649E7927C74 ON user (email)');
