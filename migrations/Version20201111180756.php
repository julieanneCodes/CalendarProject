 ...
 */
final class Version20201111180756 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TABLE user_config (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER DEFAULT NULL, color VARCHAR(10) NOT NULL COLLATE BINARY, panel_view INTEGER NOT NULL, CONSTRAINT FK_B1D83441A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user_config AS SELECT id, user_id, color, panel_view FROM user_config');
        $this->addSql('DROP TABLE user_config');
        $this->addSql('CREATE TABLE user_config (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, color VARCHAR(10) NOT NULL, panel_view INTEGER NOT NULL, user_id INTEGER NOT NULL)');
        $this->addSql('INSERT INTO user_config (id, user_id, color, panel_view) SELECT id, user_id, color, panel_view FROM __temp__user_config');
        $this->addSql('DROP TABLE __temp__user_config');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_B1D83441A76ED395 ON user_config (user_id)');
    }
